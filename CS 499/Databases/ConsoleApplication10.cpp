...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Enhanced: Better SQL injection detection
    std::string check_sql = sql;
    std::transform(check_sql.begin(), check_sql.end(), check_sql.begin(), ::tolower);

    // Check for OR attacks
    if (check_sql.find(" or ") != std::string::npos) {
        std::cout << "Suspected SQL injection - OR detected" << std::endl;
        return false;
    }

    // Check for common injection patterns
    if (check_sql.find("1=1") != std::string::npos ||
        check_sql.find("2=2") != std::string::npos ||
        check_sql.find("'='") != std::string::npos) {
        std::cout << "Suspected SQL injection - equality attack" << std::endl;
        return false;
    }

    // Check for comment injection
    if (check_sql.find("--") != std::string::npos) {
        std::cout << "Suspected SQL injection - comment detected" << std::endl;
        return false;
    }
...
// Enhanced: Safe query using prepared statement
bool run_safe_query(sqlite3* db, const std::string& username, std::vector<user_record>& records) {
    records.clear();

    // First validate the input
    if (!validate_user_input(username)) {
        std::cout << "Invalid characters in username" << std::endl;
        return false;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        records.push_back(std::make_tuple(id, name, pwd));
    }
...
void run_queries(sqlite3* db)
{
    ...
    // Enhanced: Demo the safe query
    std::cout << "\n--- Testing Safe Query ---" << std::endl;
    if (run_safe_query(db, "Fred", records)) {
        std::cout << "Safe query found " << records.size() << " users" << std::endl;
    }
...
int main()
{
    ...
    std::cout << "Enhanced Version" << std::endl;
